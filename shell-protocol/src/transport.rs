...
async fn write_frame<T>(io: &mut T, frame: &Frame) -> io::Result<()>
where
    T: FuturesAsyncWrite + Unpin,
{
    let payload = encode_frame(frame)?;
    let len = u32::try_from(payload.len())
        .map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "frame payload is too large"))?;
    io.write_all(&FRAME_MAGIC).await?;
    io.write_all(&len.to_le_bytes()).await?;
    io.write_all(&payload).await?;
    io.flush().await
}

async fn read_frame<T>(io: &mut T) -> io::Result<Option<Frame>>
where
    T: FuturesAsyncRead + Unpin,
{
