...
# A release kernel lives in the `kernel-release` directory — the image's
# own optimised cargo profile, whole-program optimised, distinct from the
# `release` the host tools use — or in `profile-use` on the target whose
# release builds read the fetched profile (docs/pgo.md, #226): the
# inspector keeps the two apart so a plain build and a profile-guided one
# never share an artifact. A kernel built against a
...
KERNEL_PATTERNS = (
    "target/*-unknown-none*/kernel-release/helios",
    "target/*-unknown-none*/profile-use/helios",
    "target/pgo-kernels/*/*-unknown-none*/profile-use/helios",
    "target/perf-baselines/worktrees/*/target/*-unknown-none*/kernel-release/helios",
