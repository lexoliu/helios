...
pub(crate) fn parse(input: &str) -> Result<ParseState> {
    let lines = input
        .lines()
        .map(str::trim)
...
pub(crate) fn needs_more_input(input: &str) -> Result<bool> {
...
#[cfg(test)]
mod tests {
    use super::{needs_more_input, parse, ParseState, Statement};
