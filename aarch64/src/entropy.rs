...
/// is seeded from a read of this device on a platform whose firmware
/// left no seed and whose processor has no `RNDR` — which is every
/// ACPI-described machine without `FEAT_RNG`. The device is programmed
/// here and read by `seed_root_entropy`; the reseed task that keeps
/// reading it needs the seeded root and so comes after.
pub(crate) fn bring_up(
    platform: &PlatformDescription,
    physical_memory_offset: usize,
    handoff: &crate::LimineBootHandoff,
) -> Option<EntropyInterrupt> {
    let entropy = discover_entropy_device(platform, physical_memory_offset, handoff);
    if entropy.is_none() {
