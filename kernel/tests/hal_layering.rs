...
#[test]
fn wasmtime_wasi_io_stays_inside_adapter_layer() {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let mut files = Vec::new();
    ...
    for path in files {
        ...
        let content = fs::read_to_string(&path)
            .unwrap_or_else(|error| panic!("failed to read {}: {error}", path.display()));
        if content.contains("wasmtime_wasi_io") || content.contains("async_trait") {
    ...
    assert!(
        leaks.is_empty(),
        "wasmtime_wasi_io and async_trait are adapter-only details:\n{}",
        leaks.join("\n")
    );
}
...
#[test]
fn wasi_protocol_names_stay_inside_wasmtime_adapter() {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let mut files = Vec::new();
    collect_files(&manifest_dir.join("src"), &mut files);
    ...
    for path in files {
        ...
        let content = fs::read_to_string(&path)
            .unwrap_or_else(|error| panic!("failed to read {}: {error}", path.display()));
        let lowered = content.to_ascii_lowercase();
        if lowered.contains("wasi") || lowered.contains("wit:") || lowered.contains("wit/") {
            leaks.push(path.display().to_string());
        }
    }
    ...
    assert!(
        leaks.is_empty(),
        "WASI/WIT protocol names are adapter-only details:\n{}",
        leaks.join("\n")
    );
}
